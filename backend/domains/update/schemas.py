...
class UpdateResponse(BaseModel):
    ...
    timestamp: datetime = Field(default_factory=datetime.now, description="When the update was performed")
    details: Optional[Dict[str, Any]] = Field(None, description="Additional details about the update")


class IngredientCacheUpdateResponse(UpdateResponse):
    """Response for ingredient cache updates."""
    
    ingredient_count: Optional[int] = Field(None, description="Number of ingredients in the updated cache")
    cache_file_path: Optional[str] = Field(None, description="Path to the updated cache file")
    last_database_update: Optional[datetime] = Field(None, description="Last database update timestamp")
