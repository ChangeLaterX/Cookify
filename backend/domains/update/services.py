    ...
    try:
        ...
        if not force and not manager.needs_update():
            ...
            return IngredientCacheUpdateResponse(
                ...
                last_database_update=datetime.fromisoformat(metadata["last_updated"]) if metadata.get("last_updated") else None,
                details={
                    "force_update": force,
        ...
        if success:
            ...
            logger.info(f"Ingredient cache updated successfully with {ingredient_count} ingredients")
...
async def force_refresh_all_caches() -> Dict[str, Any]:
    ...
    try:
        ...
        return {
            ...
            "timestamp": datetime.now(),
            "results": results,
            "message": "All caches refreshed successfully" if overall_success else "Some cache refreshes failed"
        }
