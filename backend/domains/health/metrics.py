...
class HealthMetricsCollector:
    ...
    def _update_service_metrics(self, service_name: str, metric: HealthMetric) -> None:
        ...
        if metric.response_time_ms > 0:
            service_metric.max_response_time = max(
                service_metric.max_response_time, 
                metric.response_time_ms
            )
            service_metric.min_response_time = min(
                service_metric.min_response_time, 
                metric.response_time_ms
            )
            
            # Calculate running average
            total_time = (service_metric.avg_response_time * (service_metric.total_checks - 1) + 
                         metric.response_time_ms)
    ...
    def _check_for_alerts(self, metric: HealthMetric) -> None:
        ...
        # Recovery alerts
        if (metric.status == ServiceStatus.HEALTHY and 
            service_metric.consecutive_successes == 1 and 
            service_metric.last_failure):
            alerts.append(HealthAlert(
    ...
    def get_service_history(self, service_name: str, hours: int = 1) -> List[HealthMetric]:
        ...
        cutoff_time = datetime.utcnow() - timedelta(hours=hours)
        return [
            metric for metric in self.metrics.get(service_name, [])
    ...
    def get_system_overview(self) -> Dict[str, Any]:
        ...
        avg_uptime = (
            sum(metrics.uptime_percentage for metrics in self.service_metrics.values()) /
