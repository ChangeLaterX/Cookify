...
class HealthCheckService:
    ...
    async def _check_ingredients_service(self) -> ServiceHealthStatus:
        ...
        try:
            ...
            return ServiceHealthStatus(
                ...
                details={"database": "connected", "test_query": "successful"},
            )
        except Exception as e:
            return ServiceHealthStatus(
                name="ingredients",
        ...
        try:
            ...
            if OCR_AVAILABLE:
                return ServiceHealthStatus(
                    ...
                    response_time_ms=0,
                    details={"ocr_available": "true", "provider": "Tesseract/PIL"},
                    error=None,
                )
            else:
                return ServiceHealthStatus(
                    ...
                    response_time_ms=0,
                    details={"ocr_available": "false", "provider": "Tesseract/PIL"},
                    error=None,
                )
        except Exception as e:
            return ServiceHealthStatus(
    ...
    async def _check_cache_status(self) -> ServiceHealthStatus:
        ...
        try:
            ...
            msg = (
                "Ingredient file cache is healthy"
                if file_exists and not needs_update
                else "Ingredient file cache needs update or is missing"
            )
            health_status = (
                ServiceStatus.HEALTHY
                if file_exists and not needs_update
                else ServiceStatus.DEGRADED if file_exists else ServiceStatus.UNHEALTHY
            )
        ...
        try:
            ...
            return ServiceHealthStatus(
                ...
                message=(
                    ...
                    if cache_healthy
                    else "Update service operational but cache degraded"
                ),
                details={
    ...
    def _get_enhanced_system_info(self, total_time_ms: int) -> Dict[str, str]:
        ...
        try:
            ...
            if not PSUTIL_AVAILABLE:
                basic_info["metrics_note"] = (
                    "Advanced system metrics unavailable (psutil not installed)"
                )
                return basic_info
