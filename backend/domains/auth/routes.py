...
from .schemas import (
    UserLogin, UserCreate, TokenRefresh, PasswordReset, PasswordResetConfirm,
    PasswordChange, EmailVerification, ResendVerification,
...
from .services import (
    ...
    get_user_profile, update_user_profile, AuthenticationError,
...
async def get_current_user_info(
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail={"error": "Failed to get user information", "error_code": "SERVICE_ERROR"}
        )
...
async def get_profile(
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail={"error": "Failed to get user profile", "error_code": "SERVICE_ERROR"}
        )
...
async def update_current_user_profile(
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail={"error": "Failed to update user profile", "error_code": "SERVICE_ERROR"}
        )
...
async def verify_user_email(verification_data: EmailVerification) -> MessageResponse:
    ...
    try:
        ...
        if success:
            ...
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
                detail={"error": "Email verification failed", "error_code": "VERIFICATION_FAILED"}
            )
...
async def change_user_password(
    ...
    try:
        ...
        if success:
            ...
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
                detail={"error": "Password change failed", "error_code": "CHANGE_FAILED"}
            )
