...
class AuthService:
    ...
    async def register_user(self, user_data: UserCreate) -> TokenResponse:
        ...
        try:
            ...
            self.logger.info(
                f"Supabase response - User: {auth_response.user is not None}, Session: {auth_response.session is not None}"
            )
            ...
            if not auth_response.session:
                self.logger.info(
                    f"No session returned - likely email verification required for {user_data.email}"
                )
    ...
    async def authenticate_user(self, credentials: UserLogin) -> TokenResponse:
        ...
        try:
            ...
            self.logger.error(
                f"Unexpected error during authentication for {credentials.email}: {str(e)}"
            )
...
async def update_user_profile(
    user_id: UUID, profile_data: UserProfileUpdate
) -> UserProfileResponse:
    """Update user profile."""
