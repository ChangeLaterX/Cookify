...
class AuthRateLimitMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request: Request, call_next):
        ...
        if debug_mode:
            self.logger.info(
                f"Rate limiting check - debug: {debug_mode}, enabled: {rate_limiting_enabled}, path: {request.url.path}"
            )
    ...
    def _check_rate_limit(
        self, path: str, client_key: str, current_time: float
    ) -> Tuple[bool, int]:
        """
        ...
        if len(client_data.requests) >= config.requests_per_window:
            ...
            if config.progressive_delay and client_data.violations > 1:
                progressive_delay = min(
                    ...
                    * (config.progressive_multiplier ** (client_data.violations - 1)),
