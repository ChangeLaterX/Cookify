...
class PasswordComplexityValidator:
    ...
    def analyze_password(
        ...
        if not meets_requirements["min_length"]:
            errors.append(
                f"Password must be at least {self.MIN_LENGTH} characters long (currently {len(password)})"
            )
        ...
        if not meets_requirements["unique_chars"]:
            errors.append(
                f"Password must contain at least {self.MIN_UNIQUE_CHARS} unique characters (currently {unique_chars})"
            )
        ...
        if repeated_ratio > self.MAX_REPEATED_CHAR_RATIO:
            errors.append(
                f"Password has too many repeated characters ({repeated_ratio:.1%} > {self.MAX_REPEATED_CHAR_RATIO:.1%})"
            )
        ...
        if pattern_violations:
            warnings.append(
                "Password contains some common patterns - consider making it more unique"
            )
            suggestions.append(
                "Avoid very simple sequences and common patterns for better security"
            )
        ...
        if is_common:
            warnings.append(
                "Password appears in common password lists - consider making it more unique"
            )
        ...
        if user_info is not None:
            ...
            if personal_violations:
                ...
                suggestions.append(
                    "Never use personal information in passwords - it's easily guessable"
                )
            else:
                score += 10
        ...
        core_requirements_met = (
            meets_requirements.get("min_length", False)
            and meets_requirements.get("max_length", False)
            and meets_requirements.get("char_types", False)
            and meets_requirements.get("unique_chars", False)
            and meets_requirements.get("repeated_chars", False)
        )
        ...
        if is_valid:
            ...
            if len(password) < settings.PASSWORD_RECOMMENDED_MIN_LENGTH:
                warnings.append(
                    f"Passwords {settings.PASSWORD_RECOMMENDED_MIN_LENGTH}+ characters provide better security"
                )
