...
class IngredientsTestConfig:
    ...
    VALIDATION_TESTING_ENABLED: bool = os.getenv('INGREDIENTS_TEST_VALIDATION', 'true').lower() == 'true'
...
class IngredientsTestUtils:
    ...
    def create_mock_supabase_client(success_responses: bool = True) -> Mock:
        ...
        table_mock.execute.return_value = Mock(data=table_mock.select.return_value.execute.return_value.data)
...
class MockIngredientDatabase:
    ...
    def update_ingredient(self, ingredient_id: str, data: Dict[str, Any]) -> Optional[MockSupabaseIngredient]:
