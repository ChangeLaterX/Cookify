        ...
        if hasattr(route, 'dependencies') and route.dependencies:
            for dep in route.dependencies:
                if hasattr(dep, 'dependency'):
                    ...
                    if any(keyword in dep_name.lower() for keyword in ['auth', 'token', 'user', 'security']):
                        return True
        return False
...
class TestDiscoveredEndpoints:
    ...
    def test_endpoint_accessibility(self, endpoint):
        ...
        try:
            ...
            assert response.status_code < 500, f"Endpoint {endpoint.method} {endpoint.path} returned 5xx error: {response.status_code}"
