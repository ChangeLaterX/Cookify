...
class AuthMockFactory:
    ...
    def create_supabase_response(
        ...
        if success:
            ...
            response.session.__dict__.update(session_data or AuthMockFactory.create_supabase_session_dict())
        else:
            response.user = None
...
class MockContextManager:
    ...
    def __enter__(self):
        ...
        if self.mock_dependencies:
            ...
            self.patches.append(
                patch('domains.auth.services.get_supabase_client', return_value=self._create_mock_supabase_client())
            )
            
        if self.mock_supabase:
            # Mock Supabase client
            self.patches.append(
                patch('shared.database.supabase.get_supabase_client', return_value=self._create_mock_supabase_client())
            )
    ...
    def _create_mock_supabase_client(self) -> Mock:
        ...
        if self.success_responses:
            ...
            client.auth.sign_in_with_password.return_value = AuthMockFactory.create_supabase_response()
            ...
            client.auth.sign_up.side_effect = AuthMockFactory.create_auth_error("Registration failed")
            client.auth.sign_in_with_password.side_effect = AuthMockFactory.create_auth_error("Login failed")
    ...
    def _create_mock_table(self) -> Mock:
        ...
        if self.success_responses:
            query.execute.return_value = Mock(data=[{"id": "123", "username": "testuser"}], error=None)
        else:
