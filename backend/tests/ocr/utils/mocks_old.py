...
class OCRMockFactory:
    ...
    def create_mock_ocr_service():
        ...
        mock_service._extract_price_from_text = Mock()
        mock_service._extract_quantity_and_unit_from_text = Mock()
    ...
    def create_mock_ingredient():
        ...
        mock_ingredient.ingredient_id = uuid4()
        mock_ingredient.name = "Tomatoes"
        return mock_ingredient
    
    @staticmethod
    def create_mock_search_result(ingredients: Optional[List] = None):
        """Create a mock ingredient search result."""
        if ingredients is None:
            ingredients = [OCRMockFactory.create_mock_ingredient()]
        
        mock_result = Mock()
        mock_result.ingredients = ingredients
        return mock_result
